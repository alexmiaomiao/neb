...
//二分法快速定位一个时间段的某个区块高度
func heightInPeriod(ts, te time.Time, hs, he uint64, db *storage.RocksStorage) (uint64, core.Transactions, error) {
	hm := (hs + he) / 2
...
func getTXsByDay(y,m,d int, db *storage.RocksStorage) (core.Transactions, error) {
	...
	hm, TX, err := heightInPeriod(ts, te, 2, block.Height(), db)
	if err != nil {
		return nil, err
	}
	...
	go func(hh uint64) {
		for i:=hh; ; i++ {
			block, err := getBlockByHeight(i, db)
			if err != nil {
				log.Println(err)
				continue
			}
...
func main() {
	...
	// if txs, err := getTXsByDay(2018, 6, 2, db); err != nil {
	// 	log.Fatal(err)
	// } else {
